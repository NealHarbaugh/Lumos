...
target_validation_accuracy <- 0.9999  # e.g., 99.99% accuracy
...
preprocess_image <- function(image_path, image_size) {
  ...
  # Get information about the image (e.g., dimensions, format)
  ...
  if (img_height > img_width) {
    # Rotate the image by 90 degrees if it's taller (vertical orientation)
    image <- magick::image_rotate(image, 90)
    
    # Update width and height variables after rotation
    img_width <- img_height
    img_height <- image_info$width
  }
  
  # Calculate the aspect ratio of the image (width divided by height)
  aspect_ratio <- img_width / img_height
  
  # Check if the aspect ratio of the image is greater than the target aspect 
  # ratio
  if (aspect_ratio > image_size[1] / image_size[2]) {
    ...
    # Add vertical padding to the image using the calculated values
    image <-
      magick::image_border(image, geometry = paste0("0x",
                                                    pad_top, "+0+", pad_bottom))
  } else {
    ...
    # Add horizontal padding to the image using the calculated values
    image <-
      magick::image_border(image, geometry = paste0(pad_left, "x0+", pad_right,
                                                    "+0"))
  }
    ...
    if (apply_augmentation && runif(1) < 0.5) {
      ...
      while (!augmentation_applied) {
        if (runif(1) < 0.5) {
          ...
          image_one <-
            image_noise(image_one, noisetype = "gaussian")
          augmentation_applied <- TRUE
        }
      }
    }
...
for (list_name in List_of_paths) {
  ...
  # Get the original path and expand it
  assign(path_var_name, path.expand(get(path_var_name)))
  ...
  # Create temporary folder in the original path
  dir.create(file.path(get(path_var_name), Temporary_folder),
             showWarnings = FALSE)
...
Testing_paths <- c("Eval_Color",
                   "Eval_Gray", "Eval_Binary")

for (list_name in Testing_paths) {
  total_images <- length(get(list_name))
  total_batches <- ceiling(total_images / batch_size)
  
  pbar <- progress_bar$new(total = total_batches,
                           format = "[:bar] :percent ETA: :eta",
                           show_after = 0)
  
  var_names <- get(list_name)  # Get the list of image file paths
  
  for (batch_start in seq(1, total_images, by = batch_size)) {
    ...
    temp_folder <- file.path(get(path_var_name))
    
    # Process images within the current batch and save resized images to disk
    for (i in seq_along(var_names[batch_start:batch_end])) {
      img <- preprocess_image(var_names[batch_start + i - 1], image_size)
      ...
      if (!is.null(img)) {
        ...
        if (img_width != image_size[1] ||
          ...
          file_name <- basename(var_names[batch_start + i - 1])
          file_extension <-
            tools::file_ext(var_names[batch_start + i - 1])
          file_name_without_extension <-
            tools::file_path_sans_ext(file_name)
          new_file_name <-
            paste0(file_name_without_extension,
                   "_",
                   batch_start + i - 1,
                   ".",
                   file_extension)
          
          magick::image_write(img, path = file.path(temp_folder, new_file_name))
...
total_samples <- length(Train_Color)


###-------Define separate environments for each model---------------------------
...
with(Lumos_gray_environment, {
  # Define Lumos gray model architecture
  Lumos_gray_generator_model <- keras_model_sequential() %>%
    layer_conv_2d(
      filters = 32,
      kernel_size = 3,
      activation = "relu",
      input_shape = c(image_size[1], image_size[2], 3)
    ) %>%
    ...
    layer_dense(units = prod(image_size), activation = "linear") %>%
    layer_reshape(target_shape = c(image_size[1], image_size[2], 1))
  
  # Compile the model
  optimizer_gray <- tf$keras$optimizers$Adam(learning_rate = 0.0002)
  
  Lumos_gray_generator_model %>% compile(
...
with(Lumos_binary_environment, {
  ...
  Lumos_binary_generator_model %>% compile(
...
# Append "_Temporary" to the file paths
Train_Color <- list.files(
  Train_Color_Path,
  pattern = ".jpg|.jpeg|.png",
  full.names = TRUE,
  ignore.case = TRUE
)
Train_Gray <- list.files(
  Train_Gray_Path,
  pattern = ".jpg|.jpeg|.png",
  full.names = TRUE,
  ignore.case = TRUE
...
Eval_Color <- list.files(
  Eval_Color_Path,
  pattern = ".jpg|.jpeg|.png",
  full.names = TRUE,
  ignore.case = TRUE
)
Eval_Gray <- list.files(
  Eval_Gray_Path,
  pattern = ".jpg|.jpeg|.png",
  full.names = TRUE,
  ignore.case = TRUE
...
Lumos_gray_generator_model_path <-
  path.expand(Lumos_gray_path)

# Initialize an empty data frame to store the metrics
Gray_metrics_df <- data.frame(
  epoch = numeric(0),
  accuracy = numeric(0),
...
with(Lumos_gray_environment, {
  ...
  for (epoch in 1:epochs) {
    ...
    Checkpoint_folder_location <-
      file.path(Lumos_gray_generator_model_path, Checkpoint_folder)
    ...
    for (i in 1:total_batches_needed) {
      ...
      # Generate random indices to sample from storage vaults
      sample_indices <-
        sample(Stored_image_indexs, length(Eval_Color))
    ...
    # Initialize an empty data frame to store the metrics
    Index_df <- data.frame(Index = Stored_image_indexs,
                           learning_rate = current_lr)
    
    # Append the metrics to the data frame
    Gray_metrics_df <- rbind(
      Gray_metrics_df,
      data.frame(
        epoch = epoch,
...
with(Lumos_binary_environment, {
  ...
  for (epoch in 1:epochs) {
    ...
    for (i in 1:total_batches_needed) {
      ...
      # Generate random indices to sample from storage vaults
      sample_indices <-
        sample(Stored_image_indexs, length(Eval_Gray))
...
Temporary_folder <- "Temporary_folder"
Train_Color_Path <-
  file.path(Train_Color_Path, Temporary_folder)
Train_Gray_Path <-
  file.path(Train_Gray_Path, Temporary_folder)
