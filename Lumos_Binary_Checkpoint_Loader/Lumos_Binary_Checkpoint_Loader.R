...
# Set all paths for the model to be saved to
...
Lumos_binary_generator_checkpoint <- "/Checkpoint_folder/"

Checkpoint_folder_location <- paste0(Lumos_binary_path, 
                                     Lumos_binary_generator_checkpoint)

Metrics_file <- "Binary_metrics.csv"

Binary_metrics_df <- read.csv(paste0(Checkpoint_folder_location, 
                                     Metrics_file))

if (Binary_metrics_df$epoch[length(Binary_metrics_df$epoch)] > 1)
{
...
gray_image_size <- binary_image_size <- c(image_size[1], image_size[2], 1)
...
for (x in 1:length(Stored_image_indexs))
{
  ...
  # Load unaltered grayscale and binary images from file paths
  Storage_images <-
  ...
  # Store unaltered binary image in the binary storage vault
...
Lumos_binary_generator_name <- paste0("Lumos_binary_generator_", 
                                      last_epoch_completed, ".h5")

Lumos_binary_generator_model_path <- 
  paste0(Checkpoint_folder_location, Lumos_binary_generator_name)

Lumos_binary_generator_model_path <- 
  path.expand(Lumos_binary_generator_model_path)

Lumos_binary_generator_model <- 
  keras$models$load_model(Lumos_binary_generator_model_path)
...
total_batches_needed <- ceiling(length(Train_Gray)) 
